...
context('getTokenIdByIsin', () => {
  ...
  it('should fail if ISIN contains a non-ascii character', async () => {
    expect(
      securityTokenProxy.getTokenIdByIsin("€R1234567890"),
    ).to.revertedWithCustomError(securityTokenProxy, 'InvalidIsinCodeCharacter').withArgs(0xE2);
  });
  it('should fail if ISIN contains a non-alphanumeric character', async () => {
    [..." !\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"].forEach((character:string) =>
      expect(
        securityTokenProxy.getTokenIdByIsin(`FR1234${character}67890`),
      ).to.revertedWithCustomError(securityTokenProxy, 'InvalidIsinCodeCharacter').withArgs(character.charCodeAt(0))
    );
  });
});
