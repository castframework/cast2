...
async function main() {
  ...
  const newProxyDeployData = await generateNewImplementationDeployTransaction(
    config.Contracts.ImplementationAddress,
    implInitializerCall,
  );
  ...
  const fileContentAsPOJO = {
    description:
      'Data field to be used in a transaction for the deployment of a new smartcoin Proxy',
    usedConfig: {
      ...config,
    },
...
async function generateNewImplementationDeployTransaction(
  ...
  const transactionData = await proxyFactory.getDeployTransaction(
